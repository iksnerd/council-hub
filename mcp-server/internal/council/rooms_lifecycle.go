...
func (s *Server) UpdateStatus(roomID, status string) error {
	...
	if status == "resolved" {
		...
		if err := s.DB.QueryRow(`SELECT COALESCE(tags, '') FROM rooms WHERE id = ?`, roomID).Scan(&currentTags); err == nil {
			newTags := removeTag(removeTag(removeTag(removeTag(removeTag(removeTag(currentTags, "needs-synthesis"), "stale"), "stale-pin"), "stale-plan"), "unpinned-synthesis"), "incoherent")
