...
- `lib/council_hub_ui/node_identity.ex` + `lib/council_hub_ui/cluster_manager.ex` — DHCP can reassign a long-lived container's host IP without a restart (`entrypoint.sh`'s IP auto-detect only runs on `docker run`, not on every boot), leaving `RELEASE_NODE` stale with zero visible symptoms — no crash, `/health` green, MCP tools fine, just permanently unreachable to peers under that name. `ClusterManager`'s existing `~10s` reconnect tick now also runs `NodeIdentity.status/0` (compares `Node.self()`'s host against the current outbound interface IP via a local-only UDP `connect`+`sockname`, the same trick as `ip route get 1`) and, on drift, calls `NodeIdentity.rebind/1` (stop/restart `:net_kernel` under the corrected name, preserving the cookie, then re-dials known peers) — rate-limited to one attempt per 60s. Surfaced on `/status` (badge + config-doctor line) and on `/health` (`node_identity`/`node_identity_warning`, relayed through `cluster_controller.ex` → Go's `clusterNodes()`).
