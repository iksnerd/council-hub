...
- `internal/handlers/handler_room_lifecycle.go` — `signal_status`, `bulk_status_update`, `rename_project`, `regenerate_embeddings` (on-demand backfill or `full=true` clear-and-recompute; mirrored to the UI's `/status` "Backfill missing"/"Full re-embed" buttons via `/api/ui/backfill_embeddings` in `cluster_writes.go`).
