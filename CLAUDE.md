...
- `internal/council/notebook_outline.go` — Curated notebook outlines (Phase 2): `notebooks` + `notebook_entries` CRUD, position renumbering per mutation, `GetOutline` resolves `ref` (message) and `room_ref` (live room status/topic/latest decision-action) entries at render time (transclusion — dangling refs return `RefFound=false`). Four entry kinds: `prose`, `ref`, `room_ref`, and `task` (a first-class checklist item with a three-state `status` open/doing/done, toggled by `SetTaskStatus`). Empty project = global notebook (listed in every project's view; the `current-work` work-list/dev-task-cockpit pattern).
