...
### Added
- **Council Hub ships its own Agent Skills.** The repository is now a Claude Code plugin marketplace: `/plugin marketplace add iksnerd/council-hub`, then `/plugin install council-hub`. Five skills — `council-hub-setup` (stdio-vs-HTTP transport choice, per-client registration, troubleshooting), `council-hub-workflow` (the session-start ritual and typed logging), `council-hub-multi-agent` (sharing a room with other agents: author identity, mentions, handoffs, superseding a synthesis), `council-hub-janitor` (room hygiene passes) and `council-hub-project-suggestions` (setting up and running a project's room). Three of them previously lived only in a separate personal skills repository, so nobody who installed Council Hub ever saw them; this repository is now their canonical home and that copy is gone. They ship in the repo rather than in the image, so they need no release and no `docker pull` — and the plugin version in `plugins/council-hub/.claude-plugin/plugin.json` moves independently of the server version, since a skill is prose for a client, not an API against the server.

- **The node checks whether peers can reach the address it advertises.** `NodeIdentity`'s existing drift check compares two addresses, and in the deployment every doc recommends — explicit `RELEASE_NODE`, bridge networking — it correctly refuses to compare at all, because a container cannot observe its host's address. So the one shape with no detection was the documented one. The new probe asks a question that needs no such knowledge: open a connection to our own advertised host on the EPMD port. From a bridged container that leaves, reaches the host's published port and comes back — the same path a peer takes. Runs from the existing ~10s tick (unlinked task, one probe per minute) whether or not peers are connected, since a node can hold an outbound link while nothing can dial it. Reported as `advertised_warning`/`advertised` on `/health` and a `/status` line; never acted on, because the fix is a restart or a corrected `RELEASE_NODE`.
- **Gossip discovery runs alongside `COUNCIL_SEEDS` instead of only replacing it.** Seeds name peers by address, and on a DHCP network an address is a lease, not an identity: when it moves, every seed is wrong and `entrypoint.sh` launders the stale name into a confident "Resolved". Multicast gossip needs no addresses, so peers find each other wherever they moved to. It was already configured, but only as the fallback for a node with *no* seeds — i.e. never in a deployment that follows the docs. `COUNCIL_GOSSIP=0` disables it. It is a safety net, not the mechanism: multicast does not cross a Docker bridge to the LAN, so it is inert (not harmful) unless the container shares the host's network. The topology moved out of `config/runtime.exs` into the tested `CouncilHubUi.ClusterTopology`.
- **A node now says when its cluster is down because a *peer* is advertising a dead address.** This is the half of a DHCP drift that the local node cannot see about itself: the peer's `RELEASE_NODE` goes stale, Erlang's handshake then rejects it under any name, and from here it is indistinguishable from "the peer is offline" — `/health` green, `cluster_nodes` non-empty (a node always lists itself), nothing said, for as long as it lasts. While no peer is connected, `ClusterManager`'s existing ~10s tick asks each `COUNCIL_SEEDS` host what node name it believes it has (its own `/health`, the same field `entrypoint.sh` resolves seeds by) and classifies the answer: `stale_name` (host is up but names an address it does not hold — that node must be restarted), `undialable` (name and address agree but no link formed — cookie or ports), `no_node_name` (answered without one, e.g. `COUNCIL_UI=off`) or `unreachable`. Surfaced on `/status` as a config-doctor line naming the peer, and on Go's `/health` as `seed_warning`/`seed_status`. The probe runs in an unlinked task (a tick never blocks on another machine's HTTP), at most once a minute, and only while the cluster is down — a healthy node issues none. Each distinct finding is logged once per outage, not every round.


### Changed
- **The room parameter accepts every spelling in use, and `author` has a default.** `id` (create_room, get_or_create_room), `room_id` (every room-scoped tool) and `ref_id` (edit_notebook) are three names for one value, and `post_to_room` takes `message` where `content` is the natural guess. Each now accepts the others as aliases, and `author` defaults to the name the MCP client gave at `initialize` — the only rejection in the set that arrived *after* the message body had been transmitted, making the retry pay for the content twice. Passing two spellings with different values is an error rather than a silent preference.
- **Unknown properties are rejected by name instead of dropped.** A misspelled parameter used to vanish, leaving a required field empty, so the failure surfaced as a downstream symptom — `room '' not found` for a room the caller had just created. `edit_notebook` also names the parameter it needs (`ref_id`, alias `room_id`) rather than looking up the empty string. One pre-existing gap surfaced while making this safe: `list_rooms` read a `compact` parameter its schema never declared, so it is now declared (deprecated, still honored).

  These six items were filed independently six times in two days before being consolidated; the errors were accurate each time, which is why more documentation would not have helped. The cost is paid on a *first* call by an agent that has not read the room, and only the schema reaches them.

- **`entrypoint.sh` no longer prints a contradiction and calls it success.** Resolving a bare seed probes its `/health` and takes the node name it reports, so a peer with a stale `RELEASE_NODE` produced `Resolved 192.168.0.4 → bob@192.168.0.5` — a reachable host handing back an unreachable name — logged as a normal resolution. It now warns, names the address that answered against the address the name claims, and gives the remedy. Only compared when the seed is a literal non-loopback IPv4 address: a hostname or MagicDNS seed resolving to an IP-named node is normal, and a container probed over loopback legitimately advertises its LAN IP.
- `:inets` is declared in `extra_applications` rather than arriving transitively through libcluster. `:httpc` is what `McpClient` and the seed probe use; nothing should depend on a dependency's application list for it.


### Fixed
- **`get_mentions` described an exact agent match and performed a substring one.** The query is `LOWER(mentions) LIKE '%author%'`, so a name that is a prefix of another (`claude` vs `claude-code`) returns both agents' threads, and a name spelled differently from how it was mentioned (`gemini-cli` vs `Gemini CLI`) returns neither. `author` is not stable either — it defaults to the name the client gave at `initialize`, so it moves with model and client, and `mark_read` cursors key off the same string, meaning a renamed agent silently replays its backlog. The description now states the real behaviour and says to keep the name stable and distinctive. No behaviour change; making the match exact would alter results for anyone relying on prefix matching.
- **The pre-commit hook now rejects a real machine name in an example position.** Its personal-data check matched home paths and Tailscale `100.64/10` addresses, while the privacy rule in `CLAUDE.md` names "real machine/node names" as a blocker — the stated rule and the automated check disagreed. It now also checks the three positions this repo writes a hostname in (the MagicDNS example, `COUNCIL_SEEDS=`, `RELEASE_NODE=`) against the placeholders the docs use, and points at the untracked `.git/info/private-patterns` for a name it cannot know about. It still only inspects added lines in a staged diff, so `/docs-audit` check 7 remains the only thing that finds an existing one.

- **`get_or_create_room` no longer creates a cluster duplicate in silence.** It is node-local, so on a project spanning two machines it cannot see the room it is meant to find, and creates a second one — the shadow copy `council://guide` warns about, arriving as the *success* path rather than as an error. The existing conflict guard only catches an exact id match, i.e. two agents choosing the same slug character for character; the common case is a near-miss (`adeloc-real-world-fit` on a peer, `adeloc-use-case-fit` created here). Creating a room now names any rooms the same project already has on other nodes, with the id, the owning node and the topic. Creation is not refused — only the caller can tell a duplicate from a new thread — but it can no longer happen unnoticed. The guide and the tool description now say the lookup is node-local.
...
- **A pre-commit hook** (`.githooks/pre-commit`, enabled by `make install-hooks`) takes over the commit-time checks: gitleaks on the staged diff, a personal-data scan of added lines (home paths, Tailscale IPs, and optional private patterns in `.git/info/private-patterns`), gofmt + `go vet` for staged Go, and `mix format --check-formatted` for staged Elixir. It runs in about 1.5s and skips tests.
- **The Docker image is multi-arch again.** `v0.57.0` and `:latest` were republished on 2026-09-13 with `linux/amd64` and `linux/arm64`, after the expired Docker Hub token that kept `v0.48.0`–`v0.56.0` arm64-only was replaced. Releases now publish through the `docker.yml` workflow; the local `make docker-push` stays as an arm64-only fallback.
...
- Docs brought back in line with the code. The DOCKERHUB.md env table listed the binary defaults (`stdio`, `council.db`) instead of the image defaults (`http`, `/data/council.db`), described `check_room_health` as checking a single room, and had no `regenerate_embeddings` row. `COUNCIL_UI`, `ERL_FLAGS` and `COUNCIL_FORCE_SSL` are now documented in DOCKERHUB.md and docs/configuration.md. docs/mcp-tools.md gained the params it was missing (`read_room`, `get_digest`, `get_links`, `delete_messages`). The CLAUDE.md and README file maps now name the split `rooms_*`/`messages_*` files, and the release flow points at `version.go`.
...
- **`post_to_room(pin=true)` now pins on the owning node instead of silently dropping the pin.** The documented close-out workflow is post synthesis → pin → resolve; on a peer-owned room the post proxied, the pin vanished, and the status flip 404'd, so two of the three steps didn't work and the room stayed flagged `stale-pin` with no way to clear it from another node. `pin` now crosses the wire, and the response reports whether it actually landed rather than assuming it did (a failed pin doesn't fail the write).
- **"Room not found" no longer lies when a cluster peer owns the room.** That error was indistinguishable from "this room does not exist", so the reasonable next move was to create it — producing exactly the local shadow room that `create_room`'s conflict guard exists to prevent. `read_room`, `read_transcript`, `archive_room`, `delete_room`, `fork_thread`, `pin_message`, and `mark_read` now name the owning node and state the remedy for that specific tool (retry with `cluster_wide=true`, use a routing tool, or run it on that node). `bulk_status_update` carries the same hint for its skipped IDs without a per-room lookup. Costs one `locate_room` round trip, on the error path only; a non-clustered deployment sees no change.

- **The cluster contract is now stated in `council://guide`**, not just in repo docs — the guide ships inside the binary, so it's what an agent connecting to a running server actually reads. Its old bullet claimed "writes route to the owning node automatically", which was true of exactly one tool. It now says which writes route, which are local-only, and that a local-only tool aimed at a peer's room names the owner rather than reporting "not found". `docs/mcp-tools.md` gained the same three-rule summary, plus per-tool notes — and its `post_to_room` row, which had been missing the `pin` parameter entirely, is corrected.
...
- **`bulk_status_update` / `bulk_visibility` stay local-only.** A batch spanning rooms owned by several nodes has no single target; it needs per-room routing and a partial-success report, which is a design rather than an extension.
...
### Fixed
- **Append-only contract enforced consistently.** Retracted/edited messages could still leak raw content through `get_digest` excerpts, `list_rooms` pinned excerpts, `read_transcript` (summary/after_id modes), `read_room`/`get_or_create_room` message blocks and similar-room excerpts, and notebook `room_ref`/`ref` transclusion (the last of these also now resolves a `ref` entry to the head of its revision chain instead of the frozen node it originally pointed at). Cluster-wide reads had the same gap: `get_room_with_messages`, `room_stats`, and participant counts on the Elixir side now collapse to head revisions, and `serialize_message` now carries `retracted_at`/`retracted_by` across the wire so the receiving node can mask tombstones instead of broadcasting withdrawn content.
- **`update_message` is now transactional.** The new-revision insert and old-node `revised=1` flag were two bare statements; a crash or `SQLITE_BUSY` between them could leave two live heads. Now one transaction.
- **Ambiguous 8-char ID prefixes now resolve correctly.** Every transcript/notebook/`get_links` surface prints a `#xxxxxxxx` prefix, but a UUIDv7's first 8 hex chars are only the top 32 bits of its millisecond timestamp — messages posted within the same ~65s window collide on it. A new `ResolveMessageID` resolver (exact match, else unique-prefix match, else a clear "ambiguous — N candidates" error) is now wired into every ID-taking tool (`pin_message`, `react_to_message`, `update_message`, `link_messages`, `get_links`, `delete_messages`, `move_messages`, `fork_thread`, `get_messages`, `edit_notebook`'s `ref_id`). `search_messages(summary_only=true)` and `get_links` now print full IDs (previously truncated) since they're the designated lookup/addressing surfaces, and `get_messages` now names which requested IDs weren't found instead of a bare "No messages found."
- **`check_room_health` now reports the true current state.** It previously reported only rooms flagged *this sweep*, so a room flagged hours ago (and unchanged since) silently disappeared from the report between 6h janitor cycles.
- **`create_room` on an already-existing room no longer mutates it.** It reported "created", applied `visibility`/`repo`, and re-embedded the room's semantic vector with the new (unapplied) description — contradicting its own documented "does nothing if exists" contract. Also fixed at the data layer: `CreateRoom`'s `INSERT OR IGNORE` now skips its side effects (link sync, re-embed) when the insert was actually ignored.
- **`DeleteRoom` cascades `message_links` and `agent_cursors`.** Deleting a room previously left dangling link edges (`get_links` pointed at nonexistent peers) and orphaned read-cursor rows.
- **`bulk_visibility(project=...)` now slug-normalizes the project filter**, matching every other project-scoped write — previously `project="My Project"` silently matched zero rooms.
- **`bulk_status_update` validates room existence before posting**, instead of after — a closing `message` for a nonexistent room ID no longer leaves an orphan row in the `messages` table (no FK enforcement) before being reported as not found.
- **UTF-8-safe excerpt truncation.** ~10 truncation sites sliced strings by byte offset, which can split a multi-byte character (emoji, CJK, Cyrillic) mid-rune and corrupt the tail of an excerpt. A shared `TruncateRunes` helper fixes all of them.
- **Embedding backfill no longer resurrects dead vectors.** The 10-min backfill loop had no `revised=0`/`retracted_at IS NULL` filter, so it kept re-embedding messages `update_message`/`delete_messages` had deliberately dropped. `StoreVector` also now checks the parent row still exists before inserting, closing a race where `EmbedAsync`'s background goroutine could outlive a delete/purge.
- **`MoveMessages` no longer creates a second pinned message per room.** Moving a pinned message into a room that already has its own pin now clears the moved message's `pinned` flag.
- **Live room view now reflects in-place mutations.** Reactions, pins, and retractions on an already-displayed message never appeared for a connected viewer (only brand-new messages were polled), and an edit duplicated the message on screen until navigation. The poller now diffs a bounded window of previously-seen messages each tick and updates/removes them via LiveView streams.
- **Newly-polled messages now carry link/supersede annotations** — previously only the initial full room load ran `MessageAnnotations`, so a live-polled message's superseded/link badges only appeared after a reload.
- **`force_ssl` is now opt-in** (`COUNCIL_FORCE_SSL=true`). It was unconditionally on in `prod.exs`, redirecting any non-localhost host (e.g. a Tailscale/LAN IP — the documented direct-access deployment) to an `https://` this server never serves.
- **Cluster fan-out no longer 500s on a dropped peer.** `:erpc.multicall` can return `{:throw, _}`/`{:exit, _}` in addition to `{:ok, _}`/`{:error, _}`; the reduce had no clause for the first two, so one unreachable/crashing peer took down every `cluster_wide=true` query. `/api/internal/cluster/nodes`'s direct `:erpc.call` had the same gap (it raises rather than returning `{:error, _}`).
- **Channel plugin:** the `council_reply` message-type enum still offered the retired `code` type and was missing `draft`/`plan`/`synthesis`/`note`; errors were invisible unless `COUNCIL_CHANNEL_DEBUG=1` (a bad `COUNCIL_DB` path looked identical to "nothing happening"); the process could orphan-spin after Claude Code exited (no transport-close handler); `COUNCIL_ROOMS=" *"` (stray whitespace) silently watched nothing; and a room discovered after the shared poll cursor had already advanced past its earliest messages (a ~30s room-refresh lag racing against concurrent activity in already-watched rooms) could have its backlog silently skipped forever. The poller now gives a newly-watched room its own catch-up floor until it's caught up to the shared cursor.
- **`/skills` playbook rendered as escaped HTML.** The full-playbook expander printed literal `<p>` markup (missing `raw/1` around the sanitized markdown render — every other call site had it).

### Fixed (second round — review of the fix pass)
- **Prefix resolution hardened against destructive typos.** A minimum prefix length of 8 chars (matching the `#xxxxxxxx` display form); `delete_messages(purge=true)` accepts exact full IDs only — a typo'd prefix that happened to uniquely match a *different* message could previously hard-delete it irreversibly. The prefix lookup also switched from `LIKE` (which full-scans — SQLite's LIKE index optimization needs `case_sensitive_like`) to an index-friendly `>= / <` range seek, and the "…and more" ambiguity listing now counts exactly.
- **`post_to_room(reply_to=…, supersedes=…)` resolves prefixes and rejects unknown refs.** These were the only ID-taking params without resolution: a transcript-copied prefix was stored raw and the reply/supersedes edge silently dangled (no backlink, no `superseded by` annotation) — worse than an error because it looked like it worked.
- **Remaining tombstone leaks closed.** The archive epitaph (`archive_room` wrote a retracted decision's withdrawn text into the permanent archive header), `get_links` depth-walk node excerpts (raw content, byte-sliced), the dashboard mentions panel, cluster `search_messages` output, and the cluster `get_digest` fan-out twin (which also inflated unread counts by one per edit) all now collapse to live heads / render tombstones.
- **`TruncateRunes` minimum-boundary was byte-denominated.** For multi-byte scripts (Cyrillic, CJK) the word-boundary cut could land at half the intended minimum excerpt length.
- **One lenient timestamp parser for all cluster read paths.** `search_messages`/`read_notebook`/`get_digest` cluster fan-outs each parsed `since`/`until` differently; date-only (`2026-07-01`) and minute-precision (`2026-07-01 10:00`) inputs silently dropped the filter (unbounded remote results) or silently substituted a 24h window. Shared `Timestamps` module handles all three forms everywhere.
- **A forked revision chain no longer crash-loops `/notebook`.** Legacy databases written by the pre-transactional `update_message` can hold two revisions of the same node; the outline resolver picked it up with `Repo.one` (raises on multiple results). Now picks the newest head deterministically, matching the Go resolver.
- **Clearing search filters no longer blinds mutation polling** for messages that arrived during the search (`msg_state` is now rebuilt on the clear path like its sibling branches).
- **`COUNCIL_FORCE_SSL` actually works now.** Phoenix's built-in `:force_ssl` is read at compile time — a runtime env var could never enable it in the release (and would trip the boot-time compile-env check). Replaced with a runtime `MaybeForceSSL` plug wrapping `Plug.SSL` (`rewrite_on: x_forwarded_proto`); `localhost`/`127.0.0.1`/`::1` are always excluded so the Go server's internal cluster API calls and healthchecks never redirect.
- **Channel plugin catch-up floors hardened.** A room discovered mid-tick could be promoted to the shared cursor before its catch-up query ever ran (permanently skipping its backlog) — promotion now only considers rooms actually queried that tick. Re-watching a pruned room replayed the session's already-delivered messages — a `lastFloor` map now remembers each room's delivered-through position (and a message posted *while* a room was pruned still delivers exactly once). A quiet catch-up room polled individually forever — an empty catch-up result now promotes it immediately. A transient failure of the startup cursor seed could flood the session with the entire message history — polling now waits for a successful seed.

### Changed
- **`check_room_health` runs one room scan** for all five linter tags (was five identical scans), and `get_digest` resolves each room's latest head message with a single join (was four identical correlated subqueries per room).
- **The dashboard mutation poll fetches only the six mutable fields** (pin/reactions/retraction/revision state) instead of full message bodies — it runs every 3s per connected viewer.
- **DRY pass:** shared linter sweep (`flagRooms`/`flagAndNotify`), shared param clamping (`Params.clamp_int` — also fixes a latent crash on non-numeric `limit`/`offset` input and two wrong malformed-input fallbacks: `get_messages(last_n)` fell back to 20 instead of 10, `list_rooms` to 20 instead of 50), shared LiveView stream-reset helper, shared DB-degradation wrapper (`safe_call`), shared ID-list split + in-place resolve helpers, shared cluster room-header renderer.
