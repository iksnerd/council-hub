...
| `get_digest` | `project`?, `since`?, `unread_only`?, `agent`?, `exclude_stale`?, `cluster_wide`? | Get activity feed since timestamp with health flags; use `unread_only=true` after `mark_read`; `exclude_stale=true` hides the inactive-room graveyard |
